...
This module populates the dedicated `Transfer` aesop rule set, bundling the
...
* `transfer` (exposed through the `transferTac` wrapper) registered as an
  ...
  `Transfer` set closes the quantified and function-equality transfer shapes in
...
imports). The set is therefore declared in the separate module
`AesopRuleSet`, which this module imports; the registrations and the
`aesop (rule_sets := [Transfer])` demonstrations below live here, and any file
importing this module can add further rules to the set.

### Recipe for a file that imports this module
...
failure mode the transfer layer surfaces (see `Related.lean`'s
...
absent square remains an open goal.
-/
...
/-! ## Registering the rules into the `Transfer` set
...
These run the *named* `Transfer` set directly (usable here because this file imports
